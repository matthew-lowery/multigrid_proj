...
print("Smoothing current grid")
for i in range(100): #why does python and C have different number of smoothing iterations to get to the same number?
    coarseError = solvers.GaussSolve(coarseSize,coarseError,coarseRes,[coarseDeltaX,coarseDeltaY,coarseDeltaZ])
print("Done")
...
midpoint  = int((fineSize-1)/2)
print("midpoint at 1st grid up",fineError[midpoint,midpoint,midpoint])
...
midpoint  = int((fineSize-1)/2)
print("midpoint at 2nd grid up",fineError[midpoint,midpoint,midpoint])
